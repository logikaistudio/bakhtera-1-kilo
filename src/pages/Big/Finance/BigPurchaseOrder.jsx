...
const POFormModal = ({ po, coaList, onClose, onSave }) => {
    ...
    const handleSave = async (status = 'draft') => {
        ...
        try {
            ...
            if (isEdit) {
                const { error } = await supabase.from('big_pos').update(payload).eq('id', po.id);
                if (error) throw error;
            } else {
                const { error } = await supabase.from('big_pos').insert([payload]);
                if (error) throw error;
            }
            onSave();
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-dark-surface/60 text-silver-dark text-xs uppercase">
                                <tr>
                                    ...
                                    <th className="px-3 py-2 w-48">COA Big (Beban)</th>
                            ...
                            <tbody>
                                {form.po_items.map((item, idx) => (
                                    <POItemRow key={idx} item={item} idx={idx} coaList={coaList}
                                        canEdit={canEdit('big_finance')} onChange={updateItem} onRemove={removeItem} />
                                ))}
...
const BigPurchaseOrder = () => {
    ...
    const fetchAll = async () => {
        ...
        try {
            const [{ data: poData }, { data: coa }] = await Promise.all([
                supabase.from('big_pos').select('*').order('created_at', { ascending: false }),
    ...
    const handleApprove = async (id) => {
        if (!canApprove('big_finance')) return alert('Tidak ada akses untuk menyetujui PO.');
        await supabase.from('big_pos').update({ status: 'approved', approval_status: 'approved' }).eq('id', id);
        fetchAll();
    };

    const handleReject = async (id) => {
        if (!canApprove('big_finance')) return alert('Tidak ada akses.');
        await supabase.from('big_pos').update({ status: 'rejected', approval_status: 'rejected' }).eq('id', id);
        fetchAll();
    };
    ...
    const handleDelete = async (id) => {
        ...
        await supabase.from('big_pos').delete().eq('id', id);
        fetchAll();
    };
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold gradient-text flex items-center gap-2">
                        <ShoppingCart className="w-8 h-8" /> Purchase Order Big
                    </h1>
                    <p className="text-silver-dark mt-1">Kelola pembelian dan pengeluaran Big module</p>
                </div>
                                        ...
                                        <tr key={po.id} className="hover:bg-white/5 smooth-transition">
                                            ...
                                            <td className="px-4 py-3">
                                                <div className="flex items-center justify-center gap-2">
                                                    ...
                                                    {canDelete('big_finance') && po.status === 'draft' && (
...
export default BigPurchaseOrder;
