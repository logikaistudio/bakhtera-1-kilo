    ...
    const fetchTB = async () => {
        ...
        try {
            const [{ data: accounts }, { data: entries }] = await Promise.all([
                supabase.from('bridge_coa').select('*').order('code'),
                supabase.from('bridge_journal_line_items')
                    .select('coa_id, debit, credit, bridge_journal_entries!inner(entry_date)')
            ...
            (entries || []).forEach(e => {
                ...
                const entDate = e.bridge_journal_entries?.entry_date;
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold gradient-text flex items-center gap-2">
                        <Scale className="w-8 h-8" /> Trial Balance — Bridge
                    </h1>
                    <p className="text-silver-dark mt-1">Saldo akun dan mutasi periode Bridge module</p>
                </div>
...
export default BridgeTrialBalance;
