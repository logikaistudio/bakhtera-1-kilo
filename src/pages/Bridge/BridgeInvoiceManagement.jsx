...
const InvoiceFormModal = ({ invoice, coaList, onClose, onSave }) => {
    ...
    const handleSave = async (status = 'draft') => {
        ...
        try {
            ...
            if (isEdit) {
                const { error } = await supabase.from('bridge_invoices').update(payload).eq('id', invoice.id);
                if (error) throw error;
            } else {
                const { error } = await supabase.from('bridge_invoices').insert([payload]);
                if (error) throw error;
            }
            onSave();
        } catch (err) {
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-dark-surface/60 text-silver-dark text-xs uppercase">
                                <tr>
                                    ...
                                    <th className="px-3 py-2 w-48">COA Bridge</th>
                            ...
                            <tbody>
                                {form.invoice_items.map((item, idx) => (
                                    <ItemRow key={idx} item={item} idx={idx} coaList={coaList}
                                        canEdit={canEdit('bridge_finance')}
...
const BridgeInvoiceManagement = () => {
    ...
    const fetchAll = async () => {
        ...
        try {
            const [{ data: inv }, { data: coa }] = await Promise.all([
                supabase.from('bridge_invoices').select('*').order('created_at', { ascending: false }),
    ...
    const handleDelete = async (id) => {
        if (!canDelete('bridge_finance')) return alert('Tidak ada akses untuk menghapus.');
        if (!confirm('Hapus invoice ini?')) return;
        await supabase.from('bridge_invoices').delete().eq('id', id);
        fetchAll();
    };

    const handleCancel = async (id) => {
        if (!canEdit('bridge_finance')) return alert('Tidak ada akses.');
        await supabase.from('bridge_invoices').update({ status: 'cancelled' }).eq('id', id);
        fetchAll();
    };
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold gradient-text flex items-center gap-2">
                        <FileText className="w-8 h-8" /> Invoice Bridge
                    </h1>
                    <p className="text-silver-dark mt-1">Kelola invoice tagihan Bridge module</p>
                </div>
