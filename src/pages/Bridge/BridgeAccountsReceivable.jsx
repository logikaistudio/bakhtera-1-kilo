...
import { createARPaymentJournal, createARReversalJournal, getAllCOA } from '../../utils/bridgeJournalHelper';
...
const BridgeAccountsReceivable = () => {
    ...
    const fetchARTransactions = async () => {
        try {
            ...
            // ── BLINK ONLY: Primary source = bridge_ar_transactions ──────────
            ...
            if (!arError && arRows && arRows.length > 0) {
                // ✅ Data exists in bridge_ar_transactions — use it directly
                finalRows = arRows;
            } else {
                // ⚠️ bridge_ar_transactions kosong atau error → fallback ke bridge_invoices
                if (arError) {
                    console.warn('[Bridge AR] bridge_ar_transactions error:', arError.message);
                } else {
                    console.warn('[Bridge AR] bridge_ar_transactions kosong, fallback ke bridge_invoices');
                }
...
const PaymentRecordModal = ({ ar, formatCurrency, onClose, onSuccess }) => {
    ...
    const fetchARAccounts = async () => {
        try {
            const { data } = await supabase.from('bridge_coa').select('*').eq('type', 'ASSET').order('code');
    ...
    const fetchBankAccounts = async () => {
        try {
            const { data: d1 } = await supabase.from('bank_accounts').select('*').order('display_order', { ascending: true });
            if (d1 && d1.length > 0) {
                setBankAccounts(d1);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // ── Resolve invoice ID ──────────────────────────────────────────
            // ar.invoice_id = ID invoice dari bridge_ar_transactions atau hasil fallback mapping
            // ar.id         = ID AR itu sendiri (bisa dari big_ar_transactions)
            let invoiceId = ar.invoice_id || null;

            // Jika invoice_id tidak ada, cari via invoice_number di bridge_invoices
            ...
            // Jika masih tidak ada, coba ar.id sebagai invoice id (fallback bridge_invoices path)
            if (!invoiceId) {
