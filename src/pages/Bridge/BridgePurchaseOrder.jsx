...
const insertAPTransaction = async (blinkRow, fallbackRow) => {
    try {
        if (await tryInsertTable('bridge_ap_transactions', blinkRow)) return;
        const { error } = await supabase.from('bridge_ap_transactions').insert([fallbackRow]);
...
const BridgePurchaseOrder = () => {
    ...
    const statusConfig = {
        ...
        submitted: { label: 'Pending Approval', color: 'bg-yellow-600 text-yellow-100', icon: Clock },
        approved: { label: 'Approved', color: 'bg-green-600 text-green-100', icon: CheckCircle },
        received: { label: 'Received', color: 'bg-purple-600 text-purple-100', icon: Package },
        cancelled: { label: 'Cancelled', color: 'bg-red-600 text-red-100', icon: XCircle }
    };
    ...
    const handleEditPO = (po, currentVendors) => {
        // Try to match vendor_id to bridge_business_partners by ID first, then by name
    ...
    return (
        <div className="space-y-6">
            ...
            {/* PO Table */}
            <div className="glass-card rounded-lg overflow-hidden border border-accent-orange/20">
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-gradient-to-r from-accent-orange to-accent-orange/80">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">Delivery Date</th>
                                <th className="px-4 py-3 text-right text-xs font-bold text-white uppercase tracking-wider">Tax</th>
                            ...
                            {filteredPOs.length === 0 ? (
                                ...
                                filteredPOs.map((po) => {
                                    ...
                                    return (
                                        <tr
                                            key={po.id}
                                            className="hover:bg-accent-orange/5 smooth-transition cursor-pointer border-b border-dark-border/50"
                                            onClick={() => {
                                                setSelectedPO(po);
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3 whitespace-nowrap">
                                                <span className="text-silver-light font-medium">{po.vendor_name}</span>
                                            </td>
                                            <td className="px-4 py-3 whitespace-nowrap">
                                            ...
                                            <td className="px-4 py-3 text-right whitespace-nowrap">
                                                <span className="text-silver-dark text-sm">{formatCurrency(po.tax_amount || 0, po.currency)}</span>
                                            </td>
                                            <td className="px-4 py-3 text-right whitespace-nowrap">
                                                <span className="font-bold text-silver-light">{formatCurrency(po.subtotal ?? po.total_amount, po.currency)}</span>
                                            </td>
                                            <td className="px-4 py-3 text-center whitespace-nowrap">
                                                <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold ${config?.color}`}>
    ...
    const handleAllocationSelect = async (e) => {
        ...
        if (type === 'shipment') {
            ...
            if (buyingItems.length > 0) {
                ...
                let coaMap = {};
                if (coaIds.length > 0) {
                    const { data: coaData } = await supabase.from('bridge_coa').select('id, name').in('id', coaIds);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            {vendors.length === 0 && (
                                <p className="mt-1 text-xs text-yellow-400">⚠️ No vendors loaded. Go to Bridge Master Data → Business Partners and mark partners as "Vendor".</p>
                            )}
                        </div>
                        <div>
...
const POPaymentRecordModal = ({ po, formatCurrency, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Create payment record in bridge_payments table
            const paymentData = {
