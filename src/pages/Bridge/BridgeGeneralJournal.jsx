...
const EntryFormModal = ({ coaList, onClose, onSave }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const { error: lineErr } = await supabase.from('bridge_journal_line_items').insert(lineItems);
                ...
                <div className="glass-card rounded-xl overflow-hidden">
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-dark-surface/60 text-silver-dark text-xs uppercase">
                            <tr>
                                <th className="px-3 py-2 text-left">COA Bridge *</th>
...
const BridgeGeneralJournal = () => {
    ...
    const fetchAll = async () => {
        ...
        try {
            const [{ data: ent }, { data: coa }] = await Promise.all([
                supabase.from('bridge_journal_entries').select(`*, bridge_journal_line_items(*, bridge_coa(code, name))`).order('entry_date', { ascending: false }),
    ...
    const handleDelete = async (id) => {
        ...
        await supabase.from('bridge_journal_line_items').delete().eq('journal_entry_id', id);
        await supabase.from('bridge_journal_entries').delete().eq('id', id);
        fetchAll();
    };
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold gradient-text flex items-center gap-2">
                        <BookOpen className="w-8 h-8" /> General Journal — Bridge
                    </h1>
                    <p className="text-silver-dark mt-1">Jurnal manual Bridge module</p>
                </div>
                                ...
                                <React.Fragment key={ent.id}>
                                    <tr className="hover:bg-white/5 smooth-transition cursor-pointer" onClick={() => setExpanded(expanded === ent.id ? null : ent.id)}>
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            {canDelete('bridge_finance') && ent.entry_type === 'manual' && (
                                    ...
                                    {expanded === ent.id && ent.bridge_journal_line_items?.length > 0 && (
                                        <tr>
...
export default BridgeGeneralJournal;
