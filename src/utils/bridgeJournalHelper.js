    ...
    const attempts = [
        async () => {
            const { data, error } = await supabase
                ...
                .eq('table_name', 'bridge_journal_entries')
...
export function buildJERow({
    ...
    // Hardening: all ledger postings are persisted in IDR, while foreign
    // transaction value/currency are retained for traceability.
...
function extractMissingColumnsFromError(errorMessage) {
    ...
    const schemaMatch = [...errorMessage.matchAll(/Could not find the '([^']+)' column of 'bridge_journal_entries' in the schema cache/gi)];
...
export async function ensureBlinkARTransaction(invoice) {
    ...
    const { error } = await supabase.from('bridge_ar_transactions').insert([row]);
...
export async function ensureBlinkAPTransaction(po) {
    ...
    const { error } = await supabase.from('bridge_ap_transactions').insert([row]);
    return { success: !error, error, row };
}

export async function migrateBridgeFinancialRecords() {
    const [{ data: invoiceData, error: invoiceError }, { data: poData, error: poError }, { data: journalData, error: journalError }] = await Promise.all([
        supabase.from('bridge_invoices').select('*').neq('status', 'draft').neq('status', 'cancelled'),
        supabase.from('bridge_pos').select('*').in('status', ['approved', 'received', 'paid']),
        supabase.from('bridge_journal_entries').select('reference_id,reference_type')
    ]);
