# Implementation Guide: Detail Modal dengan Edit & Delete
...
### STEP 1: Update Row onClick (Line ~1104)

**FIND THIS (around line 1101-1106):**
```javascript
                                    return (
                                        <tr
                                            key={quot.id}
...
**Change:** `onClick={() => handleEditPengajuan(quot)}` menjadi open detail modal

---

### STEP 2: Add Detail Modal (After line 1490, before closing </div>)

**PASTE THIS CODE at line 1491 (after warehouse selector modal, before Item Editor Modal):**
...
**Step 1 Location:** Line ~1104
- Cari: `onClick={() => handleEditPengajuan(quot)}`
- Di dalam tabel Pengajuan Keluar (Outbound)

**Step 2 Location:** Line ~1491
- AFTER: Warehouse selector modal closing `)}`
- BEFORE: Item Editor Modal (yang sudah ada)

---

## 🎯 Hasil Akhir:

1. ✅ Klik row → Buka detail modal
2. ✅ Modal menampilkan info lengkap pengajuan
3. ✅ Button "Edit" → Buka form edit
4. ✅ Button "Hapus" → Konfirmasi delete (menggunakan modal yang sudah ada)
5. ✅ Clean, compact UI
...
- Detail Modal menggunakan existing delete confirmation modal
- Edit akan menggunakan existing `handleEditPengajuan` function
- Delete akan menggunakan existing `deleteConfirmModal` state
- Semua handler sudah tersedia, tidak perlu tambahan logic
